...
Ensure you have Python 3.7+ installed on your system. Follow these steps to set up the virtual environment and install dependencies:

1. **Remove any existing virtual environment (optional):**
   ```bash
   rm -rf .venv
   ```
...
- Searching for Telegram channels based on keywords.
- Extracting and deduplicating channel IDs from existing output files.
...
1. **Run the main script:**
   - Using Python:
     ...
     python run.py
     ```
   - Alternatively (if `python3` is required):
     ```bash
     python3 run.py
     ```

2. **Activate the virtual environment before running the script (if not already active):**
   - On Linux/macOS:
     ```bash
     source .venv/bin/activate
     ```
...
- Ensure all necessary permissions are granted for accessing files or directories.
- For troubleshooting, review the logs or error messages generated by the script.
- Keep dependencies up-to-date using:
  ```bash
  pip install --upgrade -r requirements.txt
  ```

- If you encounter issues with Python versions or dependencies, consider using a tool like [pyenv](https://github.com/pyenv/pyenv) to manage multiple Python versions.
